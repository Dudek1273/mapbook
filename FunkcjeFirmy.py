...
def show_companies(companies):
    # Wyświetla listę klinik weteryraryjnych
    print("Kliniki weteryneryjne: ")
    for company in companies:
        print(f" {company['company_name']}")


def add_company(companies):
    # Dodaje nową klinikę do listy
    company_name = input("Podaj nazwę kliniki weterynaryjnej do dodania: ")
    company_location = input("Podaj lokalizację kliniki (miasto): ")
...
def remove_company(companies):
    # Usuwa klinike z listy
    company_name = input("Podaj nazwę kliniki weterynaryjnej do usunięcia: ")
    company_found = False
    for company in companies:
...
def update_company(companies):
    # Aktualizuje nazwę kliniki
    old_company_name = input("Podaj nazwę kliniki weteryrayjnej do aktualizacji: ")
    company_found = False
    for company in companies:
        if company['company_name'] == old_company_name:
            new_company_name = input(f"Podaj nową nazwę dla {old_company_name}: ")
            new_company_location = input(f"Podaj nową lokalizację kliniki (miasto): ")
            company['company_name'] = new_company_name
            company['company_location'] = new_company_location
            company_found = True
            print(f"Nazwa kliniki została zmieniona z {old_company_name} na {new_company_name}.")
            print(f"Lokalizacja kliniki została zmieniona na {new_company_location}.")
            break
    if not company_found:
        print(f"{old_company_name} nie znaleziono takiej kliniki na liście.")
    show_companies(companies)
