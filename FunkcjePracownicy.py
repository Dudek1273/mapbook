...
def show_workers(workers):
    # Wyświetla listę pracowników kliniki
    company_name = input("Podaj nazwę kliniki, której lista pracowników ma zostać wyświetlona: ")
    company_found = False

    # Sprawdzenie, czy klinika istnieje wśród pracowników
    for worker in workers:
        if worker['worker_company'] == company_name:
            company_found = True
            break

    # Wyświetlenie listy pracowników, jeśli klinika została znaleziona
    if company_found:
...
def add_worker(workers, companies):
    # Dodaje nowego pracownika do kliniki
    ...
    for company in companies:
        if company["company_name"] == company_name:
            ...
            workers.append({
                ...
                "animals": []  # Initial empty list of animals
            })
...
def remove_worker(workers, comapnies):
    # Pobierz nazwę kliniki i imię i nazwisko pracownika do usunięcia
    ...
    # Sprawdzenie, czy klinika istnieje
    for company in companies:
        if company['company_name'] == company_name:
            company_found = True
            break
    ...
    # Usunięcie pracownika z podanej kliniki
    for worker in workers:
...
def update_worker(workers,companies):
    # Pobierz nazwę kliniki i imię i nazwisko pracownika do aktualizacji
...
def show_animals_under_care(workers, animals):
    worker_name = input("Podaj imię i nazwisko pracownika, którego lista zwierząt ma zostać wyświetlona: ")
    worker_found = False

    for worker in workers:
        if worker['worker_name'] == worker_name:
            worker_found = True
            print(f"Lista zwierząt pod opieką {worker_name}:")
            animal_found = False
            for animal in animals:
                if animal['worker_name'] == worker_name:
                    print(f" - {animal['animal_name']}")
                    animal_found = True

            if not animal_found:
                print(f"{worker_name} nie ma żadnych zwierząt pod opieką.")
            break

    if not worker_found:
        print(f"{worker_name} nie znaleziono na liście pracowników.")
