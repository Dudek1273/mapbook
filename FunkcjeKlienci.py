...
def show_clients(clients):
    # Wyświetla listę klientów kliniki
    company_name = input("Podaj nazwę kliniki, której lista klientów ma zostać wyświetlona: ")
    company_found = False

    # Sprawdzenie, czy klinika istnieje wśród klientów
    for client in clients:
        if client['client_company'] == company_name:
            company_found = True
            break

    # Wyświetlenie listy klientów, jeśli klinika została znaleziona
    if company_found:
...
def add_client(clients, companies):
    # Dodaje nowego klienta do kliniki
...
def update_client(clients,companies):
    # Pobierz nazwę kliniki i imię i nazwisko klienta do aktualizacji
